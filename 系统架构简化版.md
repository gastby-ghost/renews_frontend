...
Art Design Pro 是一个基于 **Vue 3 + TypeScript** 的现代化企业级后台管理系统，专注于文档智能生成和素材管理。系统采用**分层架构**设计，遵循**单向数据流**原则，支持高并发访问和复杂业务逻辑处理。
...
- `src/services/` - API 服务层
  - `base/` - 基础服务类
    - `baseApiService.ts` - BaseApiService
  - 具体服务
    - `authService.ts` - 认证服务
...
export class MaterialService extends BaseApiService {
  async search(query: string): Promise<ApiResponse<Material[]>> {
    return this.get<ApiResponse<Material[]>>('/materials/search', { query })
  }
}
```
...
graph TB
    subgraph "文档生成模块"
        A[ProjectCreate 项目创建] --> B[ResearchBrief 研究简报]
        B --> C[TitleCandidate 标题候选]
        C --> D[OutlineGeneration 大纲生成]
        D --> E[ContentGeneration 内容生成]
        E --> F[DocumentExport 文档导出]
    ...
    subgraph "状态管理"
        L[documentGenerateStore] --> M[项目状态]
        L --> N[任务状态]
        L --> O[内容状态]
    end

    subgraph "服务层"
        P[documentGenerateService] --> Q[项目CRUD]
        P --> R[Agent执行]
        P --> S[内容生成]
    end
...
graph TB
    ...
    subgraph "服务层"
        M[materialService] --> N[素材CRUD]
        M --> O[批量操作]
        P[materialRelationService] --> Q[关系管理]
        R[aiService] --> S[智能搜索]
        R --> T[内容分析]
    end
...
- AI Agent 任务（Search、Scope、Content）
- 长时间运行的后端任务
- 需要实时状态更新的操作
...
```typescript
// 服务层使用
const result = await documentGenerateService.executeScopeAgentWithPolling('userId', 'projectId', {
  query: '研究主题'
})
...
3. **高度模块化**: 组件、服务、状态独立，易于扩展
4. **标准化的开发流程**: Git 规范、代码审查、自动化测试
5. **优秀的性能表现**: 代码分割、懒加载、缓存优化
6. **完善的安全措施**: 认证授权、XSS/CSRF 防护
7. **完整的 DevOps**: CI/CD 自动化、监控告警
